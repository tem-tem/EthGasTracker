...
extension Indexes {
    ...
    var normalizedEthPrice: PriceDataEntity {
        ...
        for priceIndex in filteredEthPrice {
            ...
            if let value = priceIndex.Values["price"], let priceValue = Double(value) {
    ...
    func getNormalizedEthPrice(in rate: Double) -> PriceDataEntity {
...
extension Indexes {
    ...
    typealias ListEntry = (index: Int, timestamp: Date, normal:Double, fast:Double, key: String)
    ...
    var normalizedGas: [String: NormalFast] {
        ...
        return filteredGas.reduce(into: [:]) { result, gasIndex in
            ...
            if let normalString = gasIndex.Values["normal"], let normal = Double(normalString), let fastString = gasIndex.Values["fast"],
               let fast = Double(fastString) {
                result[String(timestamp)] = NormalFast(normal: normal, fast: fast)
            } else {
                print("Failed to convert gasIndex toDouble \(gasIndex.Values)")
            }
        }
    }
    ...
    func findMinMax() -> (normalMin:Double?, normalMax:Double?, fastMin:Double?, fastMax:Double?) {
