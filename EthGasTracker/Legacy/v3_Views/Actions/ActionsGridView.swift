...
//            
////            let normal = (Float(String(format: "%.6f", (gasEntry.normal *Double(metadata.limit)) / 1e9)) ?? 0) * (ethPrices[timestamp] ?? 0)
//            let gas = historical.avg
//            let ethPrice = historical.price
//            let limit = Double(action.metadata.limit)
...
//    var actions: ActionEntityGroups
//    @Binding var selectedHistoricalData: HistoricalData?
